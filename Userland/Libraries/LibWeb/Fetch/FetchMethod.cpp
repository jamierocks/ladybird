...
JS::NonnullGCPtr<JS::Promise> fetch(JS::VM& vm, RequestInfo const& input, RequestInit const& init)
{
    ...
    //     steps:
    auto process_response = [controller, locally_aborted, promise_capability, request, response_object, &relevant_realm](JS::NonnullGCPtr<Infrastructure::Response> response) mutable {
        ...
        if (response->aborted()) {
            ...
            auto deserialized_error = Infrastructure::deserialise_a_serialised_abort_reason(relevant_realm, controller->serialised_abort_reason());
