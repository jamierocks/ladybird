...
void Document::update_for_history_step_application(JS::NonnullGCPtr<HTML::SessionHistoryEntry> entry, bool do_not_reactivate, size_t script_history_length, size_t script_history_index, Optional<Bindings::NavigationType> navigation_type, Optional<Vector<JS::NonnullGCPtr<HTML::SessionHistoryEntry>>> entries_for_navigation_api, bool update_navigation_api)
{
    ...
    if (false) {
        // 1. If navigation's activation is null, then set navigation's activation to a new NavigationActivation
        //    object in navigation's relevant realm.

        // 2. Let previousEntryIndex be the result of getting the navigation API entry index of previousEntryForActivation
        //    within navigation.
